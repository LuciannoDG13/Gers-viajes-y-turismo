<?php
session_start();
require('../../db.php');

if (!isset($_GET['id_cliente'])) {
    // Redirigir si no se pasa id_cliente
    header('Location: cliente.php');
    exit;
}

$id_cliente = $_GET['id_cliente'];

// Consulta para obtener datos del cliente junto con datos usuario si quieres
$sql = "SELECT * FROM clientes INNER JOIN usuarios ON clientes.id_usuario = usuarios.id_usuario WHERE id_cliente = '$id_cliente'";
$result = $conn->query($sql);

if ($result->num_rows == 0) {
    // No existe el cliente
    echo "Cliente no encontrado.";
    exit;
}

$datos = $result->fetch_assoc();

?>
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Modificar Cliente</title>
  <link rel="stylesheet" href="/viajes turismo/css/bootstrap.min.css" />
  <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/font/bootstrap-icons.css" rel="stylesheet" />
  <style>
    body {
      margin: 0;
      font-family: 'Segoe UI', sans-serif;
      background: linear-gradient(135deg, #eaf6fb, #d3ecf5);
      min-height: 100vh;
      color: #1d3c73;
      display: flex;
      justify-content: center;
      align-items: flex-start;
      padding-top: 40px;
    }
    .form-container {
      background: #fff;
      border-radius: 18px;
      box-shadow: 0 0 25px rgba(77, 176, 230, 0.25);
      padding: 2rem;
      width: 100%;
      max-width: 500px;
    }
    h1 {
      text-align: center;
      font-weight: bold;
      margin-bottom: 1.5rem;
      color: #1d3c73;
    }
    label {
      font-weight: 600;
      margin-bottom: 0.5rem;
      color: #1d3c73;
      display: block;
    }
    input, select {
      border-radius: 12px;
      border: 1px solid #4db0e6;
      background-color: #f0f8fc;
      color: #1d3c73;
      padding: 10px;
      font-size: 1rem;
      margin-bottom: 1rem;
      width: 100%;
    }
    .btn {
      border-radius: 12px;
      font-weight: 600;
      padding: 10px;
      width: 100%;
    }
    .btn-success {
      background-color: #38a169;
      color: white;
      border: none;
    }
    .btn-success:hover {
      background-color: #2f855a;
    }
    .btn-secondary {
      background-color: #f8f9fa;
      color: #1d3c73;
      border: 1px solid #1d3c73;
      margin-top: 10px;
      width: auto;
      padding: 8px 16px;
    }
    .btn-secondary:hover {
      background-color: #e2e6ea;
    }
    .sidebar {
      height: 100vh;
      width: 250px;
      position: fixed;
      left: 0;
      top: 0;
      background-color: #f8f5f5;
      box-shadow: 2px 0 15px rgba(77, 176, 230, 0.2);
      padding: 1rem;
      border-radius: 0 18px 18px 0;
    }
    .sidebar h4 {
      text-align: center;
      margin-bottom: 2rem;
      color: #1d3c73;
      font-weight: bold;
    }
         .logo-link {
    padding: 0;
    margin: 0;
    display: block;
}

.logo-link:hover {
    background-color: transparent;
}

    .sidebar a {
      display: flex;
      align-items: center;
      gap: 10px;
      color: #1d3c73;
      padding: 10px 15px;
      text-decoration: none;
      border-radius: 8px;
      transition: background 0.3s ease;
      margin-bottom: 10px;
    }
    .sidebar a:hover {
      background-color: #e6f3fa;
    }
  </style>
</head>
<body>

   <div class="sidebar">
    <a href="../principal.php" class="logo-link">
    <img style="width: 200px; margin-left:-20px; margin-bottom: 15px;" src="../../img/logoo.png" alt="Logo">
</a>

    <a href="../pedidos/pedidos.php"><i class="bi bi-basket-fill"></i> Pedidos</a>
    <a href="../productos/productos.php"><i class="bi bi-box-seam"></i> Productos</a>
    <a href="cliente.php"><i class="bi bi-people-fill"></i> Clientes</a>
    <a href="../usuarios/usuarios.php"><i class="bi bi-people-fill"></i> Usuarios</a>
    <a href="../reportes/reportes.php"><i class="bi bi-graph-up"></i> Reportes</a>
    <a href="../ventas/historial_ventas.php"><i class="bi bi-clock-history"></i> Historial de Ventas</a>
    <a href="../estado_cuenta/estado_cuenta.php"><i class="bi bi-cash-coin"></i> Estado de Cuentas</a>
    <a href="../../cierre_sesion.php"><i class="bi bi-box-arrow-right"></i> Cerrar sesión</a>
</div>

<div class="form-container" style="margin-left: 270px;">
  <h1>Modificar Cliente</h1>
  <form action="modificar.php" method="POST">
    <input type="hidden" name="id_cliente" value="<?php echo $datos['id_cliente']; ?>">

    <label>Nombre</label>
    <input type="text" name="nombre" value="<?php echo htmlspecialchars($datos['nombre']); ?>" required>

    <label>Dirección</label>
    <input type="text" name="direccion" value="<?php echo htmlspecialchars($datos['direccion']); ?>" required>

    <label>Teléfono</label>
    <input type="text" name="telefono" value="<?php echo htmlspecialchars($datos['telefono']); ?>" required>

    <label>DNI</label>
    <input type="text" name="dni" value="<?php echo htmlspecialchars($datos['dni']); ?>" required>

    <label>Fecha de Nacimiento</label>
    <input type="date" name="fecha_nacimiento" value="<?php echo htmlspecialchars($datos['fecha_nacimiento']); ?>" required>

    <button type="submit" class="btn btn-success">Guardar Cambios</button>
    <a href="cliente.php" class="btn btn-secondary">Cancelar</a>
  </form>
</div>

</body>
</html>
